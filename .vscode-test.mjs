...
async function createTestConfig() {
    ...
    let versionStr = packageJSON.version;
    ...
    if (vsixPath) {
        ...
        if (!path.isAbsolute(vsixPath)) {
            vsixPath = path.join(import.meta.dirname, vsixPath);
        }
        log("Installing VSIX " + vsixPath);
        installExtensions.push(vsixPath);

        // Determine version to use
        const match = /swift-vscode-(\d+.\d+.\d+(-dev)?)(-\d+)?.vsix/g.exec(
        ...
        extensionDevelopmentPath = `${import.meta.dirname}/.vscode-test/extensions/${packageJSON.publisher}.${packageJSON.name}-${versionStr}`;
        ...
        tests: [
            ...installConfigs,
            {
                ...
                mocha: {
                    ...
                    slow: 10000,
                    retries: 1,
                    reporter: path.join(import.meta.dirname, ".mocha-reporter.js"),
                    reporterOptions: {
