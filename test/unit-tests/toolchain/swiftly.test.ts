...
suite("Swiftly Unit Tests", () => {
    ...
    suite("getSwiftlyToolchainInstalls", () => {
        ...
        test("should warn and return empty array when version is null", async () => {
            ...
            const result = await Swiftly.listAvailableToolchains(mockLogger as any);

            expect(result).to.deep.equal([]);
            expect(mockUtilities.execFile).to.have.been.calledWith("swiftly", ["--version"]);
        });

        test("getListAvailableToolchains should handle errors and return empty array", async () => {
            mockedPlatform.setValue("darwin");
            const mockLogger = {
                info: () => {},
                error: () => {},
            ...
            const result = await (Swiftly as any).getListAvailableToolchains(mockLogger);
        ...
        test("getToolchainInstallLegacy should return empty array when installedToolchains is not an array", async () => {
            ...
            const result = await (Swiftly as any).getToolchainInstallLegacy();
