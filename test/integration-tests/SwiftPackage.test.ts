...
tag("medium").suite("SwiftPackage Test Suite", function () {
    ...
    suite("identity-case", () => {
        ...
        test("Identity case-insensitivity", async function () {
            const isValid = await swiftPackage.isValid;
            if (!isValid) {
                // Sometimes SPM fails to resolve swift-log with
                // Couldn’t get revision ‘1.6.2^{commit}’:
                // fatal: Needed a single revision
                // This is an issue with SPM itself so we skip in that case.
                this.skip();
            }
    ...
    suite("identity-different", () => {
        ...
        test("Identity case-different", async function () {
            const isValid = await swiftPackage.isValid;
            if (!isValid) {
                // Sometimes SPM fails to resolve swift-log with
                // Couldn’t get revision ‘1.6.2^{commit}’:
