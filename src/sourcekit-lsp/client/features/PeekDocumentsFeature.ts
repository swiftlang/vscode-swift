...
export class PeekDocumentsFeature implements StaticFeature {
    private subscriptions: Disposable[] = [];

    constructor(private readonly client: SourceKitLanguageClient) {}

    fillClientCapabilities(capabilities: ClientCapabilities): void {
        fillExperimentalCapability(this.client, capabilities, PeekDocumentsRequest.method, {
        ...
        this.subscriptions.push(
            this.client.onRequest(
                ...
                async (params: PeekDocumentsParams) => {
                    const peekURI = this.client.protocol2CodeConverter.asUri(params.uri);
                    ...
                    const peekLocations = params.locations.map(location => {
                        if (typeof location === "string") {
                            ...
                            return new vscode.Location(
                                this.client.protocol2CodeConverter.asUri(location),
                                new vscode.Position(0, 0)
                            );
                        }
                        return this.client.protocol2CodeConverter.asLocation(location);
                    });

                    await this.openPeekedEditorIn(peekURI, peekPosition, peekLocations);
     ...
     */
    private async openPeekedEditorIn(
        uri: vscode.Uri,
        ...
        await vscode.commands.executeCommand(
            ...
            new vscode.Position(
                position.line,
                position.character !== 0 ? position.character - 1 : 1
            ),
